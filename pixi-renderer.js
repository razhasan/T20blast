...
const pixiRenderer = (function () {
    ...
    async function init(containerId) {
        ...
        layerEl = container;
        // The one element the working, pre-Pixi game already renders the
        // pitch on. Instead of trusting #pixi-layer's own (CSS-derived)
        // measured size — which is what was silently drifting and pushing
        // the ball off toward the right edge of the page — every frame we
        // now force-copy this element's *actual* on-screen box (position
        // AND size) onto #pixi-layer. That guarantees pixel-for-pixel
        // parity with the surface that's known to render correctly.
    ...
    // Force #pixi-layer's box (left/top/width/height, in real CSS px) to
    // exactly match #pitch-canvas's live getBoundingClientRect(). This is
    // the actual fix for the "ball on the right side of the page" bug:
    // #pixi-layer was relying on `inset:0` + `width/height:100%` lining up
    // with #pitch-canvas by coincidence of shared CSS — any drift between
    // the two (rounding, a resize event landing between layout passes,
    // etc.) meant Pixi's own measured container size stopped matching the
    // canvas the game actually draws on, so the world scale/position was
    // computed against the wrong box and the ball rendered far outside the
    // pitch. Re-synced every frame so it can never drift again.
    function syncLayerToPitchCanvas() {
        ...
        const parentRect = offsetParent
            ...
            : canvasRect; // fallback: at least keep width/height correct
    ...
    function fitStage() {
        ...
        // IMPORTANT: use CSS/logical pixel size, not physical pixel size
        // (app.renderer.width/height, which is multiplied by
        // devicePixelRatio) — that mismatch was a previous ~2x offset bug
        // on high-DPI (retina/most phones) screens.
        ...
        // Keep Pixi's own renderer size in lockstep with the box we just
        // forced #pixi-layer into, rather than waiting on its internal
        // ResizeObserver (from `resizeTo`) to catch up on its own schedule.
        if (app.renderer &&
